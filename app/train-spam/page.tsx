                  ...
                  <div className="flex items-start gap-2 md:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {trainingData.results?.training_history &&
                        trainingData.results.training_history.train_losses.length > 0 && (
                          <div className="bg-slate-900/50 rounded-lg p-3 space-y-2 mb-4">
                            <p className="text-xs font-semibold text-slate-400">
                              Training History ({trainingData.results.training_history.train_losses.length} Epochs)
                            </p>
                            <div className="space-y-1 text-xs">
                              <p>
                                <span className="text-slate-500">Train Losses:</span>{" "}
                                <span className="font-mono text-emerald-300">
                                  {trainingData.results.training_history.train_losses
                                    .map((l) => l.toFixed(4))
                                    .join(" → ")}
                                </span>
                              </p>
                              <p>
                                <span className="text-slate-500">Train Accuracies:</span>{" "}
                                <span className="font-mono text-purple-300">
                                  {trainingData.results.training_history.train_accuracies
                                    .map((a) => `${(a * 100).toFixed(1)}%`)
                                    .join(" → ")}
                                </span>
                              </p>
                              <p>
                                <span className="text-slate-500">Val Losses:</span>{" "}
                                <span className="font-mono text-blue-300">
                                  {trainingData.results.training_history.val_losses
                                    .map((l) => l.toFixed(4))
                                    .join(" → ")}
                                </span>
                              </p>
                              <p>
                                <span className="text-slate-500">Val Accuracies:</span>{" "}
                                <span className="font-mono text-amber-300">
                                  {trainingData.results.training_history.val_accuracies
                                    .map((a) => `${(a * 100).toFixed(1)}%`)
                                    .join(" → ")}
                                </span>
                              </p>
