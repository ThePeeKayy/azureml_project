  ...
  async function getAzureMLAuthToken(): Promise<string> {
    ...
    const params = new URLSearchParams({ client_id: process.env.AZURE_CLIENT_ID!, client_secret: process.env.AZURE_CLIENT_SECRET!, grant_type: "client_credentials", scope: "https://management.azure.com/.default" });
  ...
  async function submitAzureMLJob(params: HyperParameters) {
    ...
    const jobId = `bert-spam-${Date.now()}`;
    ...
    const jobConfig = { 
      properties: { 
        jobType: "Command", 
        command: `cat > train_spam.py << 'EOFSCRIPT'\n${scriptContent}\nEOFSCRIPT\npip install transformers scikit-learn --break-system-packages && python train_spam.py --hidden-units ${params.hiddenUnits} --learning-rate ${params.learningRate} --epochs ${params.epochs} --batch-size 32`, 
        ...
        experimentName: "bert-spam-detection", 
        displayName: `DistilBERT Spam Detector - ${params.hiddenUnits} units - ${new Date().toISOString()}`, 
        description: `DistilBERT spam detection`, 
  ...
  export async function POST(request: NextRequest) {
    ...
    const { hiddenUnits, learningRate, epochs } = body;
    if (hiddenUnits === undefined || learningRate === undefined || epochs === undefined) {
      return NextResponse.json({ error: "Missing hyperparameters" }, { status: 400 });
    }
    
    const job = await submitAzureMLJob({ hiddenUnits, learningRate, epochs });
